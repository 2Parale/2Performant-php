...
class GetAffiliateProgramTest extends TestCase
{
    ...
    public function testGetAffiliateProgramByIdSendsGetRequestWithCorrectUrl(): void
    {
        $api = $this->createApiWithMockHttp([
            new Response(200, [], json_encode(['program' => ['id' => 'abc123']])),
        ]);

        $api->getAffiliateProgram($this->createMockAffiliate(), 'abc123');

        $request = $this->requestHistory[0]['request'];
        $this->assertSame('GET', $request->getMethod());
        $this->assertSame(
            '/affiliate/programs/abc123.json',
            $request->getUri()->getPath()
        );
    }

    public function testGetAffiliateProgramByIdRaisesExceptionForInvalidId(): void
    {
        $api = $this->createApiWithMockHttp([
            new Response(200, [], json_encode(['program' => ['id' => 'abc123']])),
        ]);
