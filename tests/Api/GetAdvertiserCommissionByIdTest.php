...
class GetAdvertiserCommissionByIdTest extends TestCase
{
    ...
    public function testGetAdvertiserCommissionByIdSendsGetRequestWithCorrectUrl(): void
    {
        $api = $this->createApiWithMockHttp([
            new Response(200, [], json_encode(['commission' => ['id' => 'abc123']])),
        ]);

        $api->getAdvertiserCommission($this->createMockAdvertiser(), 'abc123');

        $request = $this->requestHistory[0]['request'];
        $this->assertSame('GET', $request->getMethod());
        $this->assertSame(
            '/advertiser/programs/default/commissions/abc123.json',
            $request->getUri()->getPath()
        );
    }

    public function testGetAdvertiserCommissionByIdRaisesExceptionForInvalidId(): void
    {
        $api = $this->createApiWithMockHttp([
            new Response(200, [], json_encode(['commission' => ['id' => 'abc123']])),
        ]);
