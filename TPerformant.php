...
ini_set( 'include_path', dirname( __FILE__ ) . '/PEAR' . PATH_SEPARATOR . dirname( __FILE__ ) . PATH_SEPARATOR . ini_get( 'include_path' ) );
...
class TPerformant
{
    ...
    /* Affiliates: List campaigns and feed id and name in a nested way */
    function campaign_affiliate_feeds( )
    {
        return $this->hook( "/campaigns/product_stores/all.json", "campaigns", array( ), 'GET' );
    }
    ...
    */
    ...
    function hook( $url, $expected, $send = null, $method = 'GET', $where = 'main' )
    {
        ...
        if ( isset( $returned->error ) )
            throw new TPException_API( $this, $returned->error, null, array(
                 'request' => array(
                     $url,
                    $send,
                    $method,
    ...
    function simpleHttpRequest( $url, $params, $method )
    {
        ...
        if ( PEAR::isError( $response ) ) {
            ...
            throw new TPException_Transport( $this, $response->getReasonPhrase(), null, array(
                 'request' => $req,
                'response' => $response
            ) );
        } else {
